...
lengthV v = sqrt $ (vx v) ^ 2 + (vy v) ^ 2 + (vz v) ^ 2

normalizeV :: Vector -> Vector
normalizeV v = V ((vx v) / l) ((vy v) / l) ((vz v) / l) where l = lengthV v

dotV :: Vector -> Vector -> Double
dotV v1 v2 = (vx v1) * (vx v2) + (vy v1) * (vy v2) + (vz v1) * (vz v2)

crossV :: Vector -> Vector -> Vector
crossV v1 v2 = V ((vy v1) * (vz v2) - (vz v1) * (vy v2)) ((vz v1) * (vx v2) - (vx v1) * (vz v2)) ((vx v1) * (vy v2) - (vy v1) * (vx v2))

addV :: Vector -> Vector -> Vector
addV v1 v2 = V ((vx v1) + (vx v2)) ((vy v1) + (vy v2)) ((vz v1) + (vz v2))

subV :: Vector -> Vector -> Vector
subV v1 v2 = V ((vx v1) - (vx v2)) ((vy v1) - (vy v2)) ((vz v1) - (vz v2))

orthogonalV :: [Vector] -> Bool
orthogonalV [] = True
orthogonalV l = (orthogonalList (head l) (tail l)) && (orthogonalV (tail l))

orthogonalList :: Vector -> [Vector] -> Bool
orthogonalList v [] = True
orthogonalList v (x : xs) = (orthogonalPair v x) && (orthogonalList v xs)

orthogonalPair :: Vector -> Vector -> Bool
orthogonalPair v1 v2 = (dotV v1 v2) == 0
...
{-
 2. (4p) Liste imbricate
  ...
  (1p) flatten
-}
data SList a = Void | Cons a (SList a) | Imbricate (SList a) | Concat (SList a) (SList a) deriving Show

emptySList :: SList a
emptySList = Void

consElem :: a -> SList a -> SList a
consElem x sl = Cons x sl

consList :: SList a -> SList a -> SList a
consList sl1 sl2 = Concat sl1 sl2

headSList :: SList a -> SList a
headSList Void = Void
headSList (Cons x sl) = Cons x Void
headSList (Imbricate sl) = sl
headSList (Concat sl1 sl2) = sl1

tailSList :: SList a -> SList a
tailSList Void = Void
tailSList (Cons x sl) = sl
tailSList (Imbricate sl) = Void
tailSList (Concat sl1 sl2) = sl2

deepEqual :: Eq a => SList a -> SList a -> Bool
deepEqual Void Void = True
deepEqual Void _ = False
deepEqual _ Void = False
deepEqual (Cons x sl1) (Cons y sl2) = (x == y) && (deepEqual sl1 sl2)
deepEqual (Cons x sl1) sl2 = (deepEqual (Cons x Void) (headSList sl2)) && (deepEqual sl1 (tailSList sl2))
deepEqual (Imbricate sl1) sl2 = (deepEqual sl1 (headSList sl2)) && (deepEqual (tailSList sl2) Void)
deepEqual (Concat sl1 sl2) sl3 = (deepEqual sl1 (headSList sl3)) && (deepEqual sl2 (tailSList sl3))


flatten :: SList a -> [a]
flatten Void = []
flatten (Cons x sl) = x : (flatten sl)
flatten (Imbricate sl) = flatten sl
flatten (Concat sl1 sl2) = (flatten sl1) ++ (flatten sl2)
...
-}
data BST a = BSTNil | Tree a (BST a) (BST a) deriving Show

insertElem :: (Ord a, Eq a) => BST a -> a -> BST a
insertElem BSTNil x = Tree x BSTNil BSTNil
insertElem (Tree root t1 t2) x = if x < root then Tree root (insertElem t1 x) t2 else Tree root t1 (insertElem t2 x)

findElem :: (Ord a, Eq a) => BST a -> a -> Maybe a
findElem BSTNil x = Nothing
findElem (Tree root t1 t2) x = if root == x then Just x else if x < root then findElem t1 x else findElem t2 x

subTree :: (Ord a, Eq a) => BST a -> a -> a -> Maybe (BST a)
subTree BSTNil x y = Nothing
subTree (Tree root t1 t2) x y = let st1 = (subTree t1 x y)
                                    st2 = (subTree t2 x y)
                                    in if (not . isNothing) st1 then st1
                                        else if (not . isNothing) st2 then st2
                                          else let t1x = not . isNothing $ findElem t1 x
                                                   t1y = not . isNothing $ findElem t1 y
                                                   t2x = not . isNothing $ findElem t2 x
                                                   t2y = not . isNothing $ findElem t2 y
                                                   in if t1x && t1y then Just t1
                                                     else if t2x && t2y then Just t2
                                                       else if ((root == x) && (t1y || t2y)) || ((root == y) && (t1x || t2x)) || (t1x && t2y) || (t1y && t2x) then Just (Tree root t1 t2)
                                                         else Nothing

inorder :: BST a -> [a]
inorder BSTNil = []
inorder (Tree root t1 t2) = (inorder t1) ++ [root] ++ (inorder t2)
...
mapTree f t = Node (f (value t)) (map (mapTree f) (children t))

foldlTree :: (b -> a -> b) -> b -> Tree a -> b
foldlTree f acc t = if null (children t) then f acc (value t)
                      else foldl (foldlTree f) (f acc (value t)) (children t)
...
newtype DList a = DL [a] deriving Show
...
consDL = \x -> \(DL l) -> DL $ x : l

snocDL :: a -> DList a -> DList a
snocDL = \x -> \(DL l) -> DL $ l ++ [x]

appendDL :: DList a -> DList a -> DList a
appendDL = \(DL l1) -> \(DL l2) -> DL $ l1 ++ l2

headDL :: DList a -> Maybe a
headDL = \(DL l) -> if null l then Nothing else Just $ head l

tailDL :: DList a -> Maybe (DList a)
tailDL = \(DL l) -> if null l then Nothing else Just $ DL $ tail l
