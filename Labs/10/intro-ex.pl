...
%myConcat(X, [], X).
myConcat([], X, X).
myConcat([E|R], X, [E|C]) :- myConcat(R, X, C).
...
%myReverse(_,_):- fail.
%myReverse([], []).
%myReverse(Res , [E|R]) :- myReverse(Rev, R), myConcat(Rev, [E], Res).

myReverse([], []).
myReverse([E|R], Res) :- myReverse(R, Rev), myConcat(Rev, [E], Res).
...
myReverseAcc([E|R], Acc, Res) :- myConcat([E], Acc, NewAcc), myReverseAcc(R, NewAcc, Res).
...
extract(L, Start, _, []) :- length(L, Len),
                            Start >= Len,!.
extract(_, Start, End, []) :- Start > End,!.
extract(L, Start, End, Result) :- %length(L, Len),
                                  %Len > Start,
                                  %Start =< End,
                                  nth0(Start, L, X),
                                  NewStart is Start + 1,
                                  extract(L, NewStart, End, Rec),
                                  myConcat([X], Rec, Result).
...
factorial2(0, 1).
factorial2(N, Res) :- %N is Prev + 1, %doesn't work, order matters
                      factorial2(Prev, FactPrev),
                      N is Prev + 1,
                      Res is FactPrev * N.
...
%elem_in_list(E, [E|_]) :- !.
%elem_in_list(E, [_|R]) :- elem_in_list(E, R).

elem_in_list(E, [E|_]).
elem_in_list(E, [_|R]) :- elem_in_list(E, R).

pos_elem(E, [E|_], 0) :- !.
pos_elem(E, [_|R], N) :- pos_elem(E, R, NRec),
                         N is NRec + 1.
...
%sublista(_,_):- fail.
sublista([], []).
sublista([_|R], L) :- sublista(R, L).
sublista([E|R], [E|R1]) :- sublista(R, R1).
...
all_descendants(_, []).
all_descendants(N, [E|R]) :- all_descendants(N, R),
							 descendantOf(E, N),
							 not(elem_in_list(E, R)).


descendants_list(N, L) :- all_descendants(N, L),
                          \+ (descendantOf(X, N), not(elem_in_list(X, L))),!.

%descendants(_,_):- fail.
descendants(N, Len) :- descendants_list(N, L),
                       length(L, Len).
					   %(nod(N),
					   %descendants_list(N, L),
					   %length(L, Length),
					   %Len =:= Length).
...
%sameTree(_,_):- fail.
sameTree(X, Y) :- nod(R), descendantOf(X, R), descendantOf(Y, R),!.
sameTree(X, Y) :- X == Y,!.
sameTree(X, Y) :- descendantOf(X, Y); descendantOf(Y, X),!.
...
%drum(X, Y, [X, Y]) :- arc(X, Y); arc(Y, X),!.
%drum(X, Y, L) :- length(L, Len),
%				 Len > 2,
%				 nth0(0, L, Start),
%				 X == Start,
%				 nth0(1, L, Next),
%				 extract(L, 1, Len-1, Rec),
%				 drum(Next, Y, Rec).

%drum(X, Y, Res) :- nod(X), nod(Y),
%				   (arc(X, Y); arc(Y, X)),
%				   %arc(X, Y),
%				   Res = [X, Y],!.
%drum(X, Y, Res) :- nod(Next),
%				   (arc(X, Next); arc(Next, X)),
%				   %arc(X, Next),
%				   drum(Next, Y, Rec),
%				   not(elem_in_list(X, Rec)),
%				   Res = [X|Rec],!.


%finds a single path in the undirected graph
drum(X, Y, Res) :- drumAcc(X, Y, Res, [X]),!.
				   
drumAcc(X, Y, [X, Y], _) :- arc(X, Y); arc(Y, X),!.
drumAcc(X, Y, Res, Visited) :- nod(Next),
						       not(elem_in_list(Next, Visited)),
						       (arc(X, Next); arc(Next, X)),
							   drumAcc(Next, Y, Rec, [Next|Visited]),
							   Res = [X|Rec],!.
