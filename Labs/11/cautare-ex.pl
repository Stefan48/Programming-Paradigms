...
safe(_, []) :-!.
safe(X1/Y1/S1, [X2/Y2/S2|R]) :- safe(X1/Y1/S1, R),
								(X1 \= X2; Y1 \= Y2),
								(S1 \= S2,! ; (X1 \= X2, Y1 \= Y2),!).
...
boat(M, C) :- between(0, 2, M), between(0, 2, C),
			  M + C > 0, M + C =< 2,
			  (M >= C; M =:= 0).
...
parseState(state(Mal, M_Est, C_Est, M_Vest, C_Vest), Mal, M_Est, C_Est, M_Vest, C_Vest).
...
next_state(misionari, state(Mal1, ME1, CE1, MV1, CV1), state(Mal2, ME2, CE2, MV2, CV2)) :- boat(M, C),
																						   ((Mal1 == est, M =< ME1, C =< CE1, ME2 is ME1 - M, CE2 is CE1 - C, MV2 is 3 - ME2, CV2 is 3 - CE2);
																						   (Mal1 == vest, M =< MV1, C =< CV1, MV2 is MV1 - M, CV2 is CV1 - C, ME2 is 3 - MV2, CE2 is 3 - CV2)),
																						   opus(Mal1, Mal2),
...
preorder(X, Parc) :- get_kids(X, Kids),
					 preorder_all_elem(Kids, Rec),
					 Parc = [X|Rec].

preorder_all_elem([], []).
preorder_all_elem([E|R], Res) :- preorder(E, First),
								 preorder_all_elem(R, Others),
								 append(First, Others, Res).
								 
get_kids(Node, Kids) :- arc(Node, Kids),!.
get_kids(_, []).
...
trees(Res) :- findall(Forest, (nod(X), is_root(X), preorder(X, Forest)), Res).

is_root(X) :- \+ (nod(K), get_kids(K, Kids), member(X, Kids)).
...
span(Res) :- spanAcc([], Res).

spanAcc(Acc, Res) :- between(1, 10, X),
					 not(member(X, Acc)),
					 span_node(X, Acc, Span, NewAcc),
					 spanAcc(NewAcc, Rec),
					 Res = [Span|Rec],!.
spanAcc(_, []).
					 
span_node(X, Acc, Res, NewAcc) :- not(member(X, Acc)),
						          get_kids_nr(X, Kids),
						          span_all(Kids, [X|Acc], Rec, AccRec),
						          append([X], Rec, Res),
								  append([X|Acc], AccRec, A),
								  sort(A, NewAcc),!.
span_node(_, Acc, [], Acc).
					 
span_all([], Acc, [], Acc).
span_all([E|R], Acc, Res, NewAcc) :- span_node(E, Acc, First, FirstAcc),
									 span_all(R, FirstAcc, Others, NewAcc),
									 append_if_not_nil(First, Others, Res),!.
span_all(_, Acc, [], Acc).

append_if_not_nil([], L, L) :-!.
append_if_not_nil(L1, L2, Res) :- append([L1], L2, Res).

get_kids_nr(Node, Kids) :- edges(Node, Kids),!.
get_kids_nr(_, []).

is_root_nr(X) :- \+ (graph(NN), member(K, NN), get_kids_nr(K, Kids), member(X, Kids)).
