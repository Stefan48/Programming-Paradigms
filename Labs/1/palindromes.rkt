...
(define (how-many-bases n b)
  (if (= b 1)
     0
     (if (palindrome? (num->base n b))
        (+ 1 (how-many-bases n (- b 1)))
        (how-many-bases n (- b 1))
     )
  )
)
...
(define (first-b-pal start b)
   (if (>= (how-many-bases start 10) b)
      start
      (first-b-pal (+ 1 start) b)
   )
...
(define (longest-palindrome n)
   (if (< n 10)
       ...
       (if (palindrome? (num->base n 10))
           n
           (if (>= (length (num->base (longest-palindrome (list->num (take (num->base n 10) (- (length (num->base n 10)) 1)))) 10)) (length (num->base (longest-palindrome (list->num (cdr (num->base n 10)))) 10)))
               (longest-palindrome (list->num (take (num->base n 10) (- (length (num->base n 10)) 1))))
               (longest-palindrome (list->num (cdr (num->base n 10))))
           )
       )
   )
)
