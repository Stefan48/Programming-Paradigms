...
myCycle l =
  if null l then []
  else let myCycleHelper l1 aux = if null aux then (myCycleHelper l1 l1) else [head aux] ++ myCycleHelper l1 (tail aux) in (myCycleHelper l l)
...
randoms :: Int -> [Word8]
--randoms x = [fromIntegral (fst (next (mkStdGen x)))] ++ randoms (fst (next (mkStdGen x)))
randoms x = let randoms_helper i gen = [fromIntegral $ fst $ next gen] ++ randoms_helper i (snd $ next gen) in randoms_helper x $ mkStdGen x
...
wordToAlpha c = chr $ ((fromIntegral c) `mod` 26) + 97
{-
    5. (1p)
...
tableToFunc [] = (\c -> c)
tableToFunc bindings = let f bnd c = if null bnd then c else if c == fst (head bnd) then snd (head bnd) else f (tail bnd) c in f bindings

substCrypt :: [(Char, Char)] -> String -> String
substCrypt = map . tableToFunc
--substCrypt t s = map (tableToFunc t) s
{-
    7. (Bonus - 1p)
...
genRotTable x = zip ['a'..'z'] $ map wordToAlpha [fromIntegral x..fromIntegral (x + 25)]
...
encryptVigenere str key = let encryptVigenereHelper str1 key1 = if null str1 then [] else [tableToFunc (genRotTable (ord (head key1) - 97)) (head str1)] ++ (encryptVigenereHelper (tail str1) (tail key1)) in encryptVigenereHelper str (take (length str) (myCycle key))
{- 9, (Bonus - 2p)
...
xorStrings str1 str2 = map (\(x, y) -> chr $ xor (ord x) (ord y)) $ zip str1 str2
...
rollingXor str1 str2 = let l1 = length str1
                           l2 = length str2
                              in if l1 > l2 then xorStrings str1 $ take l1 $ myCycle str2 else if l1 < l2 then xorStrings (take l2 $ myCycle str1) str2 else xorStrings str1 str2
