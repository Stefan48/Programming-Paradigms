...
primes n = [x | x <- [2, 3 .. n], (filter (== 0) $ map (\p -> x `mod` p) [2, 3 .. (floor (sqrt (fromIntegral x)))]) == [] ]
...
group2 :: Eq a => [a] -> [[a]]
group2 [] = []
group2 l = [firstOccurences l] ++ (group2 (drop (length (firstOccurences l)) l))

firstOccurences :: Eq a => [a] -> [a]
firstOccurences [] = []
firstOccurences (x : xs) = 
	if xs == [] then
		[x]
	else if x == (head xs) then
		[x] ++ (firstOccurences xs)
	else [x]
		
--group2 (x : xs) = [(take (length (filter (== x) (x:xs))) (repeat x))] ++ (group2 (filter (/= x) xs))
...
nrOcc :: Ord a => [a] -> [(a, Int)]
nrOcc [] = []
nrOcc (x : xs) = [(x, (length (filter (== x) (x:xs))))] ++ (nrOcc (filter (/= x) xs))
...
dup :: String -> String
dup [] = []
dup l = unwords ([head (words l)] ++ [head (words l)] ++ (words (dup (unwords (tail (words l))))))
...
isIsogram (x : xs) = 
	if isLetter x then
		(filter (== (toLower x)) (map toLower xs)) == [] && isIsogram xs
	else isIsogram xs
...
arePaired xs = correctParantheses xs []

correctParantheses :: String -> String -> Bool
correctParantheses string stack =
	if string == [] then
		stack == []
	else if ((head string) == '(') || ((head string) == '[') || ((head string) == '{') then
		correctParantheses (tail string) ([head string] ++ stack)
	else if stack == [] then
		False
	else (((head stack == '(') && (head string == ')')) || ((head stack == '[') && (head string == ']')) || ((head stack == '{') && (head string == '}')))
		&& (correctParantheses (tail string) (tail stack))
...
-}
diamond :: Char -> [String]
diamond ch = (init (diamond_helper 0 ((ord ch) - 64))) ++ [(last (diamond_helper 0 ((ord ch) - 64)))] ++ (reverse (init (diamond_helper 0 ((ord ch) - 64))))

diamond_helper :: Int -> Int -> [String]
diamond_helper i n = 
	if i == n then
		[]
	else [genString i n] ++ (diamond_helper (i + 1) n)

genString :: Int -> Int -> String
genString i n = 
	if i == 0 then
		(take (n - 1) (repeat ' ')) ++ ['A'] ++ (take (n - 1) (repeat ' '))
	else (take (n - i - 1) (repeat ' ')) ++ [chr (65 + i)] ++ (take (2 * i - 1) (repeat ' ')) ++ [chr (65 + i)] ++ (take (n - i - 1) (repeat ' '))
