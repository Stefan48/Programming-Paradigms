...
(define (compute-length get-line-segment get-start-point get-end-point)
   (let ((segment (get-line-segment)))
      (let ((A (get-start-point segment)) (B (get-end-point segment)))
         (sqrt (+ (* (- (car A) (car B)) (- (car A) (car B))) (* (- (cdr A) (cdr B)) (- (cdr A) (cdr B)))))
      )
   )
)
...
(define (distance p1 p2)
   (let ((f1 (lambda () (cons p1 p2)))
         (f2 (lambda (x) p1))
         (f3 (lambda (x) p2)))
      (compute-length f1 f2 f3)
   )
)
...
(define (compute-f-with-step f a b step)
   (let iter ((next b) (result '()))
      (if (> a next)
          result
          (iter (- next step) (cons (f next) result))
      )
   )
)
...
(define (compute-perimeter points)
   (if (null? points)
       0
       (let iter((first (car points)) (points points))
           (if (null? (cdr points))
               (distance first (car points))
               (+ (distance (car points) (cadr points)) (iter first (cdr points)))
           )
       )
   )
)
...
(define (3-sequence-max numbers separator) ; without let-values
   ;(call-with-values (lambda () (splitf-at '(1 2 3 0 4 5) (lambda(x) (not (= x 0))))) list)
   (let iter ((array numbers) (Max -1000000) (Sum 0))
      (if (null? array)
         (if (> Sum Max)
             Sum
             Max
         )
         (if (= (car array) separator)
            (if (> Sum Max)
               (iter (cdr array) Sum 0)
               (iter (cdr array) Max 0)
            )
            (iter (cdr array) Max (+ (car array) Sum))
         )
      )
   )
)
...
(define (helper numbers)
   (values numbers -1000000 0)
)

(define (sequence-max numbers separator) ; with let-values (and auxiliary function helper)
   (let-values (((array Max Sum) (helper numbers)))
      (let iter((array array) (Max Max) (Sum Sum))
         (if (null? array)
            (if (> Sum Max)
               Sum
               Max
...
(define (list-num-concat numbers)
   (string->number (call-with-values (lambda () (apply values (map number->string numbers))) string-append))
)
...
(define (find-all-suffixes number)
   (let iter ((number number) (div 10) (result '()))
      (if (> div number)
         (cons (remainder number div) result)
         (iter number (* 10 div) (cons (remainder number div) result))
      )
   )
)
...
(define (run initial-state final-state next)
   (let iter ((last initial-state) (result (cons initial-state null)))
      (let ((aux (next last)))
         (if (= aux final-state)
            (reverse (cons aux result))
            (iter aux (cons aux result))
         )
      )
   )
)
...
(define (generate-number k x)
   (list->number (run x (+ (* k (- k 1)) x) (lambda (p) (+ p k))))
)

(define (list->number L)
   (foldl (lambda (x y) (+ x (* 10 y))) 0 L)
)
