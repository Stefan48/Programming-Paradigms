...
class (Ord a) => PQueue pq a where
    ...
    toList :: pq a -> [(Prio, a)]
    --toList = undefined
    toList prq = let t = top prq
                     in if isNothing t then []
                        else [fromJust $ top prq] ++ (toList $ pop prq)

    size :: pq a -> Int
    --size = undefined -- Pentru Ex 6
    size prq = let t = top prq
                   in if isNothing t then 0
                      else 1 + (size $ pop prq)
...
instance (Ord a) => PQueue ListPQ a where
    ...
    --insert = undefined
    --insert x (LPQ list) = LPQ $ x : list
    {-insert x lpq = insert_aux x lpq []

    insert_aux x (LPQ list) acc = if null list then acc ++ [x]
                                  else if fst x > fst $ head list then acc ++ [x] ++ list
                                       else insert_aux x (LPQ $ tail list) (acc ++ [head list])-}

    insert el lpq = let insert_aux x prq acc = if null prq then acc ++ [x]
                                                        else if fst x > fst (head prq) then acc ++ [x] ++ prq
                                                             else insert_aux x (tail prq) (acc ++ [head prq])
                          in LPQ $ insert_aux el (toList lpq) []

    --top = undefined
    top (LPQ list) = if null list then Nothing else Just $ head list

    --pop = undefined
    pop (LPQ list) = if null list then LPQ [] else LPQ $ tail list

    --isEmpty = undefined
    isEmpty (LPQ list) = null list
...
merge (Node rank1 (p1, v1) l1 r1) (Node rank2 (p2, v2) l2 r2) = if p1 <= p2 then merge (Node rank2 (p2, v2) l2 r2) (Node rank1 (p1, v1) l1 r1)
                                                                else let merged = merge r1 (Node rank2 (p2, v2) l2 r2)
                                                                         in if rank l1 >= rank merged then Node (1 + rank merged) (p1, v1) l1 merged
                                                                            else Node (1 + rank l1) (p1, v1) merged l1

inorder :: LeftistPQ a -> [(Prio, a)]
--inorder = undefined
inorder (Empty _) = []
inorder (Node rk (p, v) l r) = (inorder l) ++ [(p, v)] ++ (inorder r)
...
instance (Ord a) => PQueue LeftistPQ a where
    ...
    --insert = undefined
    insert pair tree = merge tree (Node 1 pair (Empty 0) (Empty 0))

    --top = undefined
    top (Empty _) = Nothing
    top (Node _ pair _ _) = Just pair
...
instance MyFoldable ListPQ where
    --foldr' = undefined
    foldr' func acc (LPQ list) = foldr func acc (map snd list)

instance MyFoldable LeftistPQ where
    --foldr' = undefined
    foldr' func acc (Empty _) = acc
    foldr' func acc (Node _ (_, v) l r) = let acc1 = foldr' func acc r -- right
                                              acc2 = func v acc1 -- root
                                              acc3 = foldr' func acc2 l -- left
                                              in acc3
...
instance MyFunctor ListPQ where
    --fmap' = undefined
    fmap' func (LPQ list) = LPQ $ map func list

instance MyFunctor LeftistPQ where
    --fmap' = undefined
    fmap' func (Empty _) = Empty 0
    fmap' func (Node rk pair l r) = Node rk (func pair) (fmap' func l) (fmap' func r)
...
instance (Show a) => Show (LeftistPQ a) where
    --show = undefined
    show tree = let show_aux (Empty _) depth = take (2 * depth) (repeat '-') ++ "empty\n"
                    show_aux (Node rk pair l r) depth = take (2 * depth) (repeat '-') ++ show pair ++ ['\n'] ++ show_aux l (1 + depth) ++ show_aux r (1 + depth)
                    in show_aux tree 1
