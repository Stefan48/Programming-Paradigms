...
(define-struct Ability (image time position next active) #:transparent)
...
(define (get-ability-active ability)
...
(define (position-abilities abilities)
   (map (lambda(ability) (if (null? (get-ability-pos ability)) (struct-copy Ability ability [position (random-position POSITION_RANGE)]) ability)) abilities)
)

; Fiecare abilitate are o funcție next care modifica stare jocului
; Compuneti toate funcțiile next în una singură
; Hint: compose
(define (compose-abilities abilities)
   (apply compose (map (lambda(ability) (get-ability-next ability)) abilities))
)
...
(define (fill-abilities initial n abilities)
   (if (> n 0)
      (let ((len (length initial)))
         (if (< len n)
            (append initial (choice-abilities (- n len) abilities))
            initial
         )
      )
      initial
