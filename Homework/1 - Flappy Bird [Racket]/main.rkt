...
(define (next-state-bird bird gravity)
   (Bird (+ (Bird-y bird) (Bird-velocity bird)) (+ (Bird-velocity bird) gravity))
)
...
(define (change current-state pressed-key)
   (if (key=? pressed-key " ")
      (struct-copy State current-state [bird (next-state-bird-onspace (get-bird current-state) (get-variables-momentum current-state))])
      current-state
   )
)
...
(define (move-pipes pipes scroll-speed)
   (map (lambda (pipe) (Pipe (- (Pipe-x pipe) scroll-speed) (Pipe-y pipe))) pipes)
)
...
(define (clean-pipes pipes)
   (filter (lambda(pipe) (>= (+ (Pipe-x pipe) pipe-width) 0)) pipes)
)
...
(define (check-ground-collision bird)
   (>= (+ (Bird-y bird) bird-height) ground-y)
)
...
(define (check-pipe-collisions bird pipes)
   (foldl (lambda (pipe acc) (or (check-pipe-collision bird pipe) acc)) #f pipes)
)

(define (check-pipe-collision bird pipe)
   (or (check-collision-rectangles
          (make-posn bird-x (Bird-y bird)) (make-posn (+ bird-x bird-width) (+ (Bird-y bird) bird-height))
          (make-posn (Pipe-x pipe) 0) (make-posn (+ (Pipe-x pipe) pipe-width) (Pipe-y pipe)))
       (check-collision-rectangles
          (make-posn bird-x (Bird-y bird)) (make-posn (+ bird-x bird-width) (+ (Bird-y bird) bird-height))
          (make-posn (Pipe-x pipe) (+ (Pipe-y pipe) pipe-self-gap)) (make-posn (+ (Pipe-x pipe) pipe-width) ground-y)))
)
...
(define (next-state state)
   (State (next-state-bird (State-bird state) (get-variables-gravity state))
          (next-state-pipes (State-pipes state) (get-variables-scroll-speed state))
          (next-variables (get-initial-variables) (State-abilities state)) ; based on initial variables
          (next-abilities (State-abilities state) (State-bird state) (get-variables-scroll-speed state))
          (next-state-score (State-score state)))
)
...
(define (draw-frame state)
   (place-active-abilities (get-abilities-active (State-abilities state))
      (place-image bird (+ bird-x (quotient bird-width 2)) (+ (Bird-y (get-bird state)) (quotient bird-height 2))
         (place-image ground (quotient scene-width 2) (- scene-height (quotient ground-height 2))
            (place-image (score-to-image (get-score state)) text-x text-y
               (place-pipes (get-pipes state)
                  (place-visible-abilities (get-abilities-visible (State-abilities state)) initial-scene))))))
)

; Folosind `place-image/place-images` va poziționa pipe-urile pe scenă.
(define (place-pipes pipes scene)
  (foldl (lambda(pipe acc)
            (place-image (rectangle pipe-width pipe-height "solid" "green") (+ (Pipe-x pipe) (quotient pipe-width 2)) (- (Pipe-y pipe) (quotient pipe-height 2))
               (place-image (rectangle pipe-width pipe-height "solid" "green") (+ (Pipe-x pipe) (quotient pipe-width 2)) (+ (Pipe-y pipe) pipe-self-gap (quotient pipe-height 2))
                  acc))
         ) scene pipes)
)
...
(define fast-ability (Ability (hourglass "mediumseagreen") 10 null (lambda(variables) (struct-copy Variables variables [scroll-speed (max 5 (- (Variables-scroll-speed variables) 1))])) #f))
...
(define slow-ability (Ability (hourglass "tomato") 30 null (lambda(variables) (struct-copy Variables variables [scroll-speed (+ 1 (Variables-scroll-speed variables))])) #f))
...
(define (get-variables-gravity state)
   (Variables-gravity (State-variables state))
)
(define (get-variables-momentum state)
   (Variables-momentum (State-variables state))
)
(define (get-variables-scroll-speed state)
   (Variables-scroll-speed (State-variables state))
)
...
(define (clean-abilities abilities)
   (filter (lambda(ability) (>= (posn-x (get-ability-pos ability)) -20)) abilities) ; + 20 == ability-image-width / 2
)
...
(define (move-abilities abilities scroll-speed)
   (map (lambda (ability)
              (letrec ((old_pos (get-ability-pos ability)) (new_pos (make-posn (- (posn-x old_pos) scroll-speed) (posn-y old_pos))))
                 (Ability (get-ability-image ability) (get-ability-time ability) new_pos (get-ability-next ability) #f)
              )
        )
   abilities)
)
...
(define (time-counter abilities)
   (filter (lambda(ability) (>= (get-ability-time ability) 0))
      (map (lambda (ability)
                 (letrec ((old_time (get-ability-time ability)) (new_time (- old_time (/ 1 fps))))
                    (Ability (get-ability-image ability) new_time null (get-ability-next ability) #t)
                 )
           )
      abilities)
   )
...
(define (next-abilities-visible visible scroll-speed)
   (position-abilities (fill-abilities (clean-abilities (move-abilities visible scroll-speed)) DISPLAYED_ABILITIES ABILITIES))
)
...
(define (next-abilities abilities bird scroll-speed)
   (let ((visible (get-abilities-visible abilities)) (active (get-abilities-active abilities)))
      (map (lambda(ability)
             (if (and (not (get-ability-active ability)) (check-ability-collision bird ability))
                 (Ability (get-ability-image ability) (- (get-ability-time ability) (/ 1 fps)) null (get-ability-next ability) #t)
                 ability
             )
          )
         (append (next-abilities-visible visible scroll-speed) (time-counter (reverse active))))
   )
)

(define (check-ability-collision bird ability)
   (let ((x (posn-x (get-ability-pos ability))) (y (posn-y (get-ability-pos ability))))
      (check-collision-rectangles
         (make-posn bird-x (Bird-y bird)) (make-posn (+ bird-x bird-width) (+ (Bird-y bird) bird-height))
         (make-posn (- x 20) (- y 20)) (make-posn (+ x 20) (+ y 20))
      )
   )
)
...
(define (next-variables variables abilities)
   (let ((active (get-abilities-active abilities)))
      (if (null? active)
         variables
         ((compose-abilities active) variables)
      )
   ) 
)
...
(define (place-visible-abilities abilities scene)
   (foldr (lambda(ability acc)
             (place-image (get-ability-image ability) (posn-x (get-ability-pos ability)) (posn-y (get-ability-pos ability)) acc)
          )
      scene abilities)
)
...
(define (place-active-abilities abilities scene)
   (let iter ((abilities abilities) (index 0))
      (if (null? abilities)
         scene
         (place-image (scale 0.75 (get-ability-image (car abilities))) (- (posn-x abilities-posn) (* 50 index)) (posn-y abilities-posn) (iter (cdr abilities) (+ 1 index)))
      )
   )
)
